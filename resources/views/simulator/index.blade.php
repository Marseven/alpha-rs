...
<script language="JavaScript">
    function afficherservice(){
        @foreach ($services as $service)
            var service_{{$service->id}} = document.getElementById("service-{{$service->id}}");

            if (document.request.service_id.value == {{$service->id}})
            {
                service_{{$service->id}}.style.display = "block";
            }else{
                service_{{$service->id}}.style.display = "none";
            }
        @endforeach
    }
</script>
