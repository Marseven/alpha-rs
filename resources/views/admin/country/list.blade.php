    ...
    <div class="mx-auto max-w-[1400px] space-y-8">
        ...
        <div class="flex flex-wrap items-end justify-between gap-4">
            <div>
                ...
                <h2 class="mt-2 font-display text-2xl font-extrabold text-ink">Pays</h2>
                <p class="mt-1 text-sm text-ink-muted">Gestion des pays de destination.</p>
            </div>
            <x-ui.button href="{{ url('admin/list-towns') }}" variant="outline">Gérer les villes</x-ui.button>
        </div>
                    ...
                    <form action="{{ url('admin/country') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="grid gap-4 sm:grid-cols-2">
                            ...
                            <div class="sm:col-span-2">
                                <label class="mb-1.5 block text-sm font-semibold text-ink">Drapeau</label>
        ...
        <div class="rounded-2xl border border-line bg-white shadow-card">
            <div class="flex items-center justify-between border-b border-line px-6 py-4">
                <h3 class="font-display text-base font-bold text-ink">Liste des Pays</h3>
                <span class="text-sm text-ink-muted">{{ count($countries) }} pays</span>
            </div>
                        ...
                        @forelse ($countries as $country)
                            ...
                            <tr class="border-b border-line-subtle last:border-0 hover:bg-canvas">
                                <td class="px-6 py-3.5">
                                    <img src="{{ asset($country->flag) }}" alt="" class="h-9 w-9 rounded-full border border-line">
                                </td>
                                <td class="px-6 py-3.5 font-medium text-ink">{{ $country->label }}</td>
                                <td class="px-6 py-3.5 font-mono text-[13px] text-ink-muted">{{ $country->code }}</td>
                                <td class="px-6 py-3.5">
                                    <span class="inline-flex items-center rounded-full border px-3 py-1 text-xs font-bold {{ $pill }}">{{ $status['message'] ?? '—' }}</span>
                                </td>
                                <td class="px-6 py-3.5 text-right">
                                    <a href="#edit-country-{{ $country->id }}" class="text-sm font-semibold text-primary-600 hover:text-primary-700">Gérer</a>
                                </td>
                            </tr>
                        @empty
                            <tr><td colspan="5" class="px-6 py-8 text-center text-ink-muted">Aucun pays pour le moment.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
            ...
            <div class="rounded-2xl border border-line bg-white shadow-card">
                ...
                <div>
                    @foreach ($countries as $country)
                        <details id="edit-country-{{ $country->id }}" class="group border-b border-line-subtle last:border-0">
                            <summary class="flex cursor-pointer list-none items-center justify-between gap-4 px-6 py-4">
                                <span class="flex items-center gap-3">
                                    <img src="{{ asset($country->flag) }}" alt="" class="h-8 w-8 rounded-full border border-line">
                                    <span class="font-semibold text-ink">{{ $country->label }}</span>
                                </span>
                                <svg class="h-5 w-5 shrink-0 text-primary-600 transition-transform duration-200 group-open:rotate-180" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="m6 9 6 6 6-6"/></svg>
                            </summary>
                            <div class="space-y-5 border-t border-line-subtle bg-canvas px-6 py-5">
                                <form action="{{ url('admin/country/' . $country->id) }}" method="POST" enctype="multipart/form-data">
                                    ...
                                    <div class="grid gap-4 sm:grid-cols-2">
                                        <div>
                                            <label class="mb-1.5 block text-sm font-semibold text-ink">Libellé</label>
                                            <input type="text" value="{{ $country->label }}" name="label"
                                        ...
                                        <div>
                                            <label class="mb-1.5 block text-sm font-semibold text-ink">Code</label>
                                            <input type="text" value="{{ $country->code }}" name="code"
                                                class="w-full rounded-lg border-[1.5px] border-line-strong bg-white px-3.5 py-2.5 text-[15px] text-ink focus:border-primary-600 focus:ring-2 focus:ring-primary-600/15">
                                        </div>
                                        <div>
                                            <label class="mb-1.5 block text-sm font-semibold text-ink">Drapeau</label>
                                ...
                                <div class="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-line bg-white px-4 py-3">
                                    <p class="text-sm text-ink-muted">Êtes-vous sûr de vouloir supprimer ce pays ?</p>
