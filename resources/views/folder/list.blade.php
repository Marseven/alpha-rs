    ...
    <div class="mx-auto max-w-[1400px] space-y-6">
        ...
        <div class="flex flex-wrap items-end justify-between gap-3">
            <div>
                ...
                <p class="mt-1 text-sm text-ink-muted">Suivez l'avancement de vos demandes de prise en charge.</p>
            </div>
            @if ($folders->count())
                <span class="inline-flex items-center gap-2 rounded-full border border-line bg-white px-3.5 py-1.5 text-sm font-semibold text-ink-muted shadow-card">
                    <span class="font-display text-base font-extrabold text-ink">{{ $folders->count() }}</span>
                    dossier{{ $folders->count() > 1 ? 's' : '' }}
                </span>
            @endif
        </div>
                    ...
                    <table data-datatable class="w-full text-left text-sm">
                        ...
                        <tbody>
                            @foreach ($folders as $folder)
                                @php
                                    $st = App\Http\Controllers\Controller::status($folder->status);
                                ...
                                <tr class="border-b border-line-subtle last:border-0 hover:bg-canvas">
                                    <td class="px-6 py-3.5 font-mono text-[13px] font-semibold text-primary-600">{{ $folder->reference }}</td>
                                    <td class="px-6 py-3.5 font-medium text-ink">{{ trim($folder->firstname . ' ' . $folder->lastname) ?: '—' }}</td>
                                    <td class="px-6 py-3.5 text-ink-muted">{{ $folder->service->label ?? '—' }}</td>
                                    <td class="px-6 py-3.5 text-ink-muted">{{ $folder->created_at?->format('d/m/Y') }}</td>
                                    <td class="px-6 py-3.5 font-medium text-ink">{{ $folder->price ? number_format($folder->price, 0, ',', ' ') . ' XAF' : '—' }}</td>
            ...
            @else
                <div class="flex flex-col items-center justify-center px-6 py-16 text-center">
                    <div class="flex h-14 w-14 items-center justify-center rounded-2xl bg-primary-50 text-primary-600">
                        <svg class="h-6 w-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><path d="M4 4h6l2 2h8a2 2 0 0 1 2 2v10a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2Z"/></svg>
                    </div>
                    <h3 class="mt-4 font-display text-lg font-bold text-ink">Aucun dossier pour le moment</h3>
                    <p class="mt-1 max-w-sm text-sm text-ink-muted">Vos demandes de prise en charge apparaîtront ici dès leur création.</p>
                    <a href="{{ route('quote') }}" class="mt-5"><x-ui.button variant="primary">Nouvelle demande</x-ui.button></a>
                </div>
            @endif
        </div>
